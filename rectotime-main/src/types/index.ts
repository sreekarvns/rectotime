...
export interface StressActionPlan {
  title: string;
  description: string;
  frequency: string;
  minutes: number;
...
export interface StressImpactRange {
  min: number;
  max: number;
  window_days: number;
}

export interface StressPredictionV2 {
  stress_level: number;
  message: string;
  risk_band?: 'low' | 'moderate' | 'high';
  key_drivers?: string[];
  actions?: StressActionPlan[];
  expected_impact_range?: StressImpactRange;
}
