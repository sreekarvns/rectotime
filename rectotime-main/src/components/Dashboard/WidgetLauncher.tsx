      ...
      >
        ...
        <AnimatePresence>
          {showMenu && (
            <motion.div
              ...
              className="absolute bottom-14 right-0 min-w-[180px] rounded-xl border border-[color:var(--border-color)] bg-[var(--bg-secondary)] p-2 shadow-2xl"
            >
              <div className="mb-1 px-3 py-1 text-xs font-medium text-[var(--text-secondary)]">
                Widgets
              </div>
              {widgets.map((widget) => (
                <button
        ...
        <motion.button
          ...
          className={`w-14 h-14 rounded-full shadow-lg flex items-center justify-center transition-all ${
            activeWidget || currentVideoId
              ...
              : 'border border-[color:var(--border-color)] bg-[var(--bg-secondary)] text-[var(--text-primary)]'
          }`}
          title="Open widgets"
        >
          {activeWidget ? (
        ...
        {(activeWidget || currentVideoId) && (
          <motion.div
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
            className="absolute -top-1 -right-1 w-4 h-4 rounded-full border-2 border-[var(--bg-primary)] bg-green-500"
          />
        )}
      </div>
    </>
...
const WeatherWidgetInline: React.FC = () => {
  ...
  return (
    <div className="w-72 rounded-xl border border-[color:var(--border-color)] bg-[linear-gradient(180deg,var(--bg-secondary),var(--bg-tertiary))] p-4 shadow-2xl">
      {/* Current Time */}
      <div className="text-center mb-4">
        <p className="text-sm text-[var(--text-secondary)]">Current Time</p>
        <p className="text-3xl font-bold tracking-wider text-[var(--text-primary)]">{currentTime}</p>
      </div>
      ...
      <div className="border-t border-[color:var(--border-color)] pt-3">
        {weather ? (
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="font-medium text-[var(--text-primary)]">{weather.temperature}°C</p>
                  <p className="text-xs text-[var(--text-secondary)]">{weather.condition}</p>
                </div>
              </div>
              <div className="text-right">
                <p className="text-xs text-[var(--text-secondary)]">{weather.location}</p>
              </div>
            </div>
            <button
              onClick={() => setShowLocationPicker(true)}
              className="flex w-full items-center justify-center gap-2 rounded-lg border border-[color:var(--border-color)] bg-[var(--bg-tertiary)] px-3 py-2 text-sm font-medium text-[var(--text-primary)] transition hover:bg-[var(--bg-secondary)]"
            >
          ...
          <div className="text-center space-y-3">
            <p className="py-2 text-sm text-[var(--text-secondary)]">No location set</p>
            <button
              onClick={() => setShowLocationPicker(true)}
        ...
        {showLocationPicker && (
          ...
          >
            <motion.div
              ...
              className="flex max-h-[500px] w-80 flex-col rounded-xl border border-[color:var(--border-color)] bg-[var(--bg-secondary)] p-5 shadow-2xl"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-bold text-[var(--text-primary)]">Select Location</h3>
                <button
                  onClick={() => {
                    setShowLocationPicker(false);
                    setSearchQuery('');
              ...
              <div className="mb-4 relative">
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-[color:var(--border-color)] bg-[var(--bg-tertiary)] py-2 pl-10 pr-4 text-[var(--text-primary)] placeholder:text-[var(--text-secondary)] focus:border-blue-500 focus:outline-none"
                />
                {searching && (
                ...
                {searchQuery && searchResults.length > 0 ? (
                  searchResults.map((result, idx) => (
                    <button
                      ...
                      disabled={loading}
                      className="w-full rounded-lg border border-[color:var(--border-color)] bg-[var(--bg-tertiary)] p-3 text-left font-medium text-[var(--text-primary)] transition hover:bg-[var(--bg-secondary)] disabled:opacity-50"
                    >
                      <div className="font-semibold">{result.name}</div>
                      <div className="text-xs text-[var(--text-secondary)]">{result.country}</div>
                    </button>
                  ))
                ) : searchQuery && !searching ? (
                  <div className="text-center py-4 text-sm text-[var(--text-secondary)]">No cities found</div>
                ) : !searchQuery ? (
                  <>
                    <div className="px-2 py-1 text-xs font-semibold uppercase text-[var(--text-secondary)]">
                      Popular Locations
                    </div>
