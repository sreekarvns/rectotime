...
type ViewType = 'dashboard' | 'calendar' | 'timetable' | 'analytics' | 'integrations' | 'settings';
...
const Sidebar: React.FC<SidebarProps> = memo(({ currentView, onViewChange, isVideoBackground = false }) => {
  ...
  const menuItems: Array<{
    ...
    },
    {
      ...
      description: 'Connected apps & wearables',
    },
    {
      id: 'settings',
      label: 'Settings',
        ...
        {menuItems.map(item => {
          ...
          return (
            <motion.button
              key={item.id}
              onClick={() => handleViewChange(item.id)}
              className={`relative w-full text-left px-4 py-3 rounded-lg overflow-hidden ${
                isActive
                  ? isVideoBackground ? 'text-white' : 'text-white'
                  : isVideoBackground
                  ? 'text-white/80'
            ...
            >
              ...
              {isActive && (
                <motion.span
                  ...
                  className="absolute inset-0 rounded-lg bg-accent-blue shadow-lg"
              ...
              {!isActive && (
                <motion.span
                  className={`absolute inset-0 rounded-lg opacity-0 ${
                    isVideoBackground ? 'bg-white/10' : 'bg-gray-100 dark:bg-gray-700'
                  }`}
              ...
              {isActive && (
                <motion.span
                  ...
                  className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-6 bg-white/70 rounded-r-full"
                  transition={{ type: 'spring', stiffness: 380, damping: 30 }}
                />
              )}
